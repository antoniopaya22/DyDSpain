      ...
      >
                ...
                <View className="flex-row items-center">
                  ...
                  >
                    <Ionicons
                      ...
                      size={28}
                      color={selectedClass === cls.id ? cls.color : "#666699"}
                    />
                  </View>
        ...
        {showDetails && currentClassData && (
          <View className="px-5">
            ...
            <View className="mb-5">
              ...
              <View className="flex-row">
                {currentClassData.savingThrows.map((save, idx) => (
                  <View
                    key={save}
                    className="bg-surface-card border border-surface-border rounded-xl px-4 py-3 mr-2 items-center min-w-[100px]"
                  >
                    <Ionicons
              ...
              <View className="flex-row flex-wrap">
                {currentClassData.armorProficiencies.length > 0 ? (
                  currentClassData.armorProficiencies.map((armor, idx) => (
                    ...
                    >
                      <Text className="text-dark-200 text-sm">{armor}</Text>
                    </View>
                  ))
                ) : (
                  <View className="bg-dark-700 rounded-full px-3 py-1.5">
              ...
              <View className="flex-row flex-wrap">
                {currentClassData.weaponProficiencies.map((weapon, idx) => (
                  <View
                    ...
                    className="bg-surface-card border border-surface-border rounded-full px-3 py-1.5 mr-2 mb-2"
                  >
                    <Text className="text-dark-200 text-sm">{weapon}</Text>
                  </View>
                ))}
              </View>
            </View>
                ...
                <View className="flex-row flex-wrap">
                  {currentClassData.toolProficiencies.map((tool, idx) => (
                    <View
                      ...
                      className="bg-surface-card border border-surface-border rounded-full px-3 py-1.5 mr-2 mb-2"
                    >
                      <Text className="text-dark-200 text-sm">{tool}</Text>
                    </View>
                  ))}
                  {currentClassData.toolChoices && (
                ...
                >
                  ...
                  <View className="mt-3 flex-row flex-wrap">
                    {currentClassData.spellcastingAbility && (
                      <View className="bg-dark-700 rounded-full px-2.5 py-1 mr-2 mb-1">
                        <Text className="text-dark-200 text-xs">
                          Aptitud:{" "}
                          {
                            ABILITY_NAMES[
                    ...
                    {currentClassData.spellsAtLevel1 > 0 && (
                      <View className="bg-dark-700 rounded-full px-2.5 py-1 mr-2 mb-1">
                        <Text className="text-dark-200 text-xs">
                          Conjuros nv.1:{" "}
                          {currentClassData.spellsAtLevel1}
                        </Text>
                      </View>
                    )}
            ...
            {/* Subclass info */}
            <View className="bg-surface-card border border-gold-600/20 rounded-card p-4 mb-5">
              <View className="flex-row items-center mb-2">
                <Ionicons
                  ...
                  color="#d4a017"
                />
                <Text className="text-gold-400 text-sm font-bold ml-2">
            ...
            <View className="mb-5">
              ...
              {currentClassData.equipmentChoices.map((choice, idx) => (
                <View
                  key={idx}
                  className="bg-surface-card border border-surface-border rounded-card p-3 mb-2"
                >
              ...
              {currentClassData.defaultEquipment.length > 0 && (
                <View className="bg-surface-card border border-surface-border rounded-card p-3 mb-2">
                  <Text className="text-dark-300 text-xs font-semibold uppercase tracking-wider mb-2">
                    Siempre incluido
                  </Text>
                  {currentClassData.defaultEquipment.map((item, idx) => (
                    <View key={idx} className="flex-row items-center mb-1">
                      ...
                      <Text className="text-dark-200 text-sm">{item}</Text>
                    </View>
                  ))}
                </View>
              )}
